...
function buildLevelsFromSignal(signal, priceNow, support, resistance) {
  ...
  if (signal === "BUY") {
    sl = support - range * 0.1; // sedikit di bawah support
    tp1 = entry + range * 0.5;
    tp2 = entry + range * 0.8;
  } else if (signal === "SELL") {
    sl = resistance + range * 0.1; // sedikit di atas resistance
    tp1 = entry - range * 0.5;
    tp2 = entry - range * 0.8;
  }
...
btnGenerate.addEventListener("click", async () => {
  ...
  const mode = modeSelect.value; // "conservative" | "moderate" | "aggressive"
  const nowStr = getNowString();

  // 1) Ambil harga XAU dari API
  const priceInfo = await fetchXauPrice();
...
// =========================
// 7) Ambil harga sekali saat halaman dibuka
// =========================
fetchXauPrice();
